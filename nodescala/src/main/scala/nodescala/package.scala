...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs.foldRight(Future(Nil:List[T])) { (f, fs2) => for(hd <- f ; tl <- fs2) yield  hd :: tl}
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val tmp = CancellationTokenSource()
      f(tmp.cancellationToken)
      tmp
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def now: T = {
      try {
        Await.result(f, 0.second) // does not wait
      }
      catch {
        case t : Exception => throw new NoSuchElementException
      }
    }
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] =
      {
        val prom = Promise[S]()
        f.onComplete{
          case Success(_) => prom.success(cont(f))
          case Failure(exc) =>  prom.failure(exc)
        }
        prom.future
      }
